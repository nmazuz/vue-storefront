...
const STOREFRONT_REMOTE_BACKEND_URL = 'http://localhost:8080'
...
class Backend extends Abstract {
  ...
  cloneRepository () {
    return new Promise((resolve, reject) => {
      ...
      console.log(this.answers.git_path)
  ...
  createConfig () {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        let website = config.magento2.url = urlParser(this.answers.m2_url).hostname

        if (!host.length) {
          throw new Error()
        }

        config.imageable.whitelist.allowedHosts.push(website)
        ...
        config.magento2.imgUrl = urlParser(this.answers.m2_url).href + '/media/catalog/product'
...
class Storefront extends Abstract {
  ...
  npmBuild () {
    return new Promise((resolve, reject) => {
      ...
      console.log(Abstract.storefrontLogStream);
...
if (require.main.filename === __filename) {
  ...
  program
    ...
    .option('--isRemoteBackend', '')
    .option('--imagesEndpoint', '')
    .option('--backendDir', '')
    .option('--m2ApiOauth2', '')
    .option('--m2Url', '')
    .option('--m2ApiUrl', '')
    .option('--m2ApiConsumerKey', '')
    .option('--m2ApiConsumerSecret', '')
    .option('--m2ApiAccessToken', '')
    .option('--m2ApiAccessTokenSecret', '')
    .option('--locale', '')
    .parse(process.argv)
   ...
   //  var args = process.argv.slice(2);
    ...
    defaultConfig.is_remote_backend = args.isRemoteBackend === 'true'
    defaultConfig.images_endpoint = `${STOREFRONT_REMOTE_BACKEND_URL}/img/`
    defaultConfig.backend_dir = args.backendDir
    defaultConfig.m2_api_oauth2 = args.m2ApiOauth2 === 'true'
    defaultConfig.m2_url = args.m2Url
    defaultConfig.git_path = 'git';
    defaultConfig.m2_api_url = args.m2ApiUrl
    defaultConfig.m2_api_consumer_key = args.m2ApiConsumerKey
    defaultConfig.m2_api_consumer_secret = args.m2ApiConsumerSecret
    defaultConfig.m2_api_access_token = args.m2ApiAccessToken
    defaultConfig.m2_api_access_token_secret = args.m2ApiAccessTokenSecret
    defaultConfig.locale = args.locale
